...
const originFixture = (): Promise<string> =>
  gitRepo("sync", { "boomfile.toml": `[[section]]\nname = "x"\n` });

// The env the engine clones, pulls and commits with. The engine's own commit path (sync.ts's
// --commit half) shells `git commit` with no identity of its own — it relies on the machine's
// at runtime, which a CI runner may not have — so the git sandbox's identity rides in here.
const stateEnv = async (): Promise<Record<string, string>> => ({
  ...gitEnv,
  XDG_STATE_HOME: await tmp("sync"),
  NO_COLOR: "1",
});
...
test("verify reports 0 drift right after linking", async () => {
  ...
  const { ctx, out } = fakeCtx(env, repo);
...
test("sync --commit commits local edits first, then rebases them onto the pulled remote", async () => {
  ...
  expect(git(repo, "log", "-1", "--format=%s")).toBe("test commit");
...
test("doctor reports the linked config repo as reachable", async () => {
  ...
  const env = await stateEnv();
  const repo = await linkRemoteConfigRepo(env, origin);
  const { ctx, out } = fakeCtx({ ...env, BOOM_OS: "linux" }, repo);
  await doctor(ctx);
