...
// two call sites each carried their own hand-rolled component-wise version — the second one's
...
//     quietly produced NaN (and every comparison false). One of them read a version out of
//     *another machine's* JSON summary, so a malformed value is genuinely reachable — the catch
//     is what keeps a corrupt version string from crashing the caller. 0 ("no opinion") is the
