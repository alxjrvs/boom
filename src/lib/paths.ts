...
// without depending on the engine. That is the point: these four helpers used to live in
...
//     backups/<run-id>/ files displaced by an overwrite, so rollback can restore
...
// The pre-sqlite manifest path — retained only to import a legacy TSV once (see engine/state.ts).
